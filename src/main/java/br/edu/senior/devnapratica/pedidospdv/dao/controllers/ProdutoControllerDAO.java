...
import br.edu.senior.devnapratica.pedidospdv.dao.services.ProdutoServiceDAO;

@Deprecated
@RestController
public class ProdutoControllerDAO {

	@Autowired
	private ProdutoServiceDAO produtoService;

	@GetMapping("/v0/produtos")
	public ResponseEntity<List<Produto>> listar() {
		List<Produto> produtos = produtoService.buscarTodos();
		return ResponseEntity.ok(produtos);
	}

	@PostMapping("/v0/produtos")
	public ResponseEntity<Produto> salvar(@RequestBody Produto produto) {
		Produto produtoSalvo = produtoService.salvar(produto);
		return new ResponseEntity<>(produtoSalvo, HttpStatus.CREATED);
	}

	@GetMapping("/v0/produtos/{produtoId}")
	...
	@PutMapping("/v0/produtos/{produto.id}")
	...
	@DeleteMapping("/v0/produtos/{produtoId}")
